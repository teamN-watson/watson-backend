...
class Assistant():
    ...
    def search_tag(self, request, query):
        ...
        if request.user.steamId:
            # 리뷰 쓴 유저일 때
            if Steam.objects.filter(user_id=request.user.id, is_review=1).exists(): 
                tag_id = get_interest(request)
                app_id = Review.objects.filter(
            ...
            elif Steam.objects.filter(user_id=request.user.id, is_playtime=1).exists(): 
                tag_id = get_interest(request)
                app_id = Playtime.objects.filter(
                    user_id=request.user.id).values_list('app_id', flat=True)
                for i in app_id:
                    game_tag = self.get_game_tag(i)
                    if game_tag:
    ...
    def summary_game_review(self, review):
        """
        리뷰 내용 요약하는 AI 모델
        """
        prompt = """
        당신은 유저의 리뷰들을 한 눈에 깔끔하게 요약하는 도우미입니다.
        주어지는 입력 중 "리뷰"은 게임에 대한 다국어 리뷰 모음입니다.
    ...
    def search_game_review(self, base_url, cursor):
        """
        Steam API로 리뷰 데이터 수집 (유용한 순, 약 5달 이내, 최대 200개)
        """
        reviews = []
        for i in range(0,2):
            # URL 업데이트
        ...
        summary_review = self.summary_game_review(reviews)

        return summary_review

    def get_game_review_summary(self, appid):
        """
        긍정, 부정 별로 최근 유용한 리뷰 200개 요약 내용 추출하는 함수
        """
        cursor = 'cursor=*'
