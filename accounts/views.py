...
def get_recommended_games(request):
    ...
    # 기본 게임 필터링 (나이 제한, metacritic 점수 있음)
    ...
    if user.steamId:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 보유 게임 제외하고 필터링
                base_games = base_games.exclude(appID__in=owned_game_ids)
        except Exception as e:
            print(f"스팀 게임 목록 조회 실패: {str(e)}")

    # 게임 정보 가져오기
    base_games = base_games.values(
    ...
    # 2. 플레이타임 기반 추천 (스팀 연동된 경우에만 실행)
    if user.steamId and played_game_tags:
        playtime_based_games = []
        for game in base_games:
            game_tags = json.loads(game['tags']) if isinstance(game['tags'], str) else game['tags']
            game_genres = json.loads(game['genres']) if isinstance(game['genres'], str) else game['genres']
    ...
    recommended_playtime_games = heapq.nlargest(15, playtime_based_games, key=lambda x: x['score']) if playtime_based_games else []
    ...
    return Response({
        ...
        'interest_tags': list(user_tags),
        'playtime_tags': list(played_game_tags) if played_game_tags else []
    }, status=status.HTTP_200_OK)
